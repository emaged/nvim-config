-- NOTE: Local override for nvim-html-css LSP module.
-- This shadows the plugin's lua/html-css/lsp.lua via runtimepath.
-- Remove if upstream fixes htmldjango handling.
local cache = require "html-css.cache"
local utils = require "html-css.utils"

local LSP = {}

-- Local override to fix htmldjango node lookup while preserving its parser
local function get_buf_lang(bufnr)
    local ft = vim.bo[bufnr].filetype
    return vim.treesitter.language.get_lang(ft) or "html"
end

local function get_context_node(bufnr, position)
    return vim.treesitter.get_node({
        lang = get_buf_lang(bufnr),
...
local function create_server(dispatchers)
    ...
    function srv.request(method, params, callback)
        ...
        elseif method == "textDocument/completion" then
            ...
            local ft = vim.bo[bufnr].filetype
            if not utils.is_lang_enabled(ext, opts.enable_on) and not utils.is_lang_enabled(ft, opts.enable_on) then
                callback(nil, { items = {} })
                return true, 1
            end
